...
//  const float D2 =   0.01; // for 14 Bit DEGC
  const float D2 =   0.04; // for 12 Bit DEGC
...
*/
  const float C1  = -4;        // for V3 sensors
  const float C2 =  0.6480;        // for V3 sensors, 8-bit precision
  const float C3 =  -7.2e-4;     // for V3 sensors, 8-bit precision
  const float T1 =  0.01;      // for 8 Bit @ 5V
  const float T2 =  0.00128;   // for 8 Bit @ 5V
...
void SHT1x::shiftOutD(uint8 dataPin, uint8 clockPin, uint8 bitOrder, uint8 val) {
    int i;
...
void SHT1x::sendCommandSHT(int _command, int _dataPin, int _clockPin)// send a command to the SHTx sensor 
{
  ...
  shiftOutD(_dataPin, _clockPin, MSBFIRST, _command);
...
void SHT1x::waitForResultSHT(int _dataPin)// wait for the SHTx answer 
{
  ...
  if (ack == HIGH) {
    Serial.println("Ack Error Wait"); // Can't do serial stuff here, need another way of reporting errors
  }
}
////////////////////////////////////////////////////////////////////////
