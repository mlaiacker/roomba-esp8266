...
void setup(void)
{
  ...
  mqttClient.setOptions(65,false,2000);
...
//    strLog += ("mqtt("+mqttBroker+")...");
    ...
    if(mqttClient.connected())
    {
      ...
      strLog +="connected!\n";
/*      mqttClient.subscribe(espName+"/cmd/PumpA",1);
      mqttClient.loop();
      delay(10);
      mqttClient.subscribe(espName+"/cmd/PumpB",1);
      mqttClient.loop();
      delay(10);
      mqttClient.subscribe(espName+"/cmd/PumpC",1);
      mqttClient.loop();
      delay(10);*/
      mqttClient.subscribe(espName+"/cmd/Start",1);
...
void mqttReceived(String &topic, String &payload) {
  ...
  if(topic==(espName+"/cmd/PumpA") && payload.toInt()==1)
  {
    state_led=1;
    start();
  }
  if(topic==(espName+"/cmd/PumpB") && payload.toInt()==1)
  {
    state_led=2;
    start();
  }
  if(topic==(espName+"/cmd/PumpC") && payload.toInt()==1)
  {
    state_led=4;
    start();
  }
  ...
  {
    state_led=payload.toInt();
    start();
  }
  if(topic==(espName+"/cmd/Stop"))
  {
    stop();
  }
}
...
void start()
{
  ...
  {
    state_led &= ~(2);
    if(jsonSettings["DurationB"].as<int>()*rate>0)
    {
  ...
  {
    state_led &= ~(4);
    if(jsonSettings["DurationC"].as<int>()*rate>0)
    {
...
void stop()
{
  ...
  jsonData["ADC"]   = adc;
  jsonData["Fan"] = 0;
  strLog += String(hour()) + ":" + String(minute()) + ":" + String(second()) + " Stop:" + state_led + " adc:"+adc+"\n";;
...
void tick_read() {
  ...
  heat_count++;
  heat_sum += sensor_min;
  jsonData["Heat"] = heat_sum/(float)heat_count; // mean temp (called every minute)
  jsonData["Rate"] = getEvaporRate(heat_sum/(float)heat_count);
  ...
  if(humidity>100 || humidity<=0 || tempC<-100 || tempC>200)
  {
    ...
    if ((1 << ((weekday() - 2) % 7)) & clean_daily_mask)
    {
      if (hour() == clean_hour)
      {
        if ((humidity > settings_thresh || settings_thresh == -1) &&  started_by_timer == 0 && jsonData["Heat"].as<float>()>=5 )
        {
...
void mqttJsonPub(JsonObject &json, String name)
{
  if(mqttClient.connected())
  {
    for (JsonObject::iterator it = json.begin(); it != json.end(); ++it)
    {
      if(it->value.as<String>().length()>0)
      {
        mqttClient.publish(espName+"/"+name+"/"+it->key,it->value.as<String>(),true,0);
        delay(10);
      }
    }
    String str;
...
bool saveConfig() {
  ...
  mqttJsonPub(jsonSettings, "settings");
  return true;
}
